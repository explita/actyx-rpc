...
* **`removeItem(index | predicate)`**: Removes an item from the cache.
* **`updateItem(index | predicate, updater)`**: Updates an item in the cache.
* **`prependItem(item)`**: Inserts a new item at the beginning of the first page.
* **`appendItem(item)`**: Inserts a new item at the end of the last page.
* **`insertItem(index, item)`**: Inserts an item at a specific flattened index.
