...
> = {
  ...
  /**
   * Whether to refetch cached data on mount
   * - `true`: Refetch if data is stale
   * - `false`: Never refetch if cached data exists
...
export type InfiniteQueryResult<TPage, TFullPage = InfiniteQueryPage<TPage>> = {
  ...
  /** Whether the initial fetch is in progress (no data fetched yet) */
  isLoading: boolean;
...
> = {
  ...
  /**
   * Whether the query is currently loading (initial fetch or refetch on empty data).
   */
  isLoading: boolean;
