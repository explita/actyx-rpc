...
export function useQueries(
  ...
  const fetchOne = useCallback(
    async (index: number, opts?: { skipStaleCheck?: boolean }) => {
      ...
      const enabled = q.enabled ?? defaults?.enabled ?? true;
      ...
      if (!opts?.skipStaleCheck) {
        ...
        const staleTime = parseWindow(q.staleTime ?? defaults?.staleTime);
        const refetchOnMount =
          q.refetchOnMount ?? defaults?.refetchOnMount ?? true;
      ...
      const keepPrev = q.keepPreviousData ?? defaults?.keepPreviousData ?? true;
      queryClient.setQueryState(key, {
        isFetching: true,
        ...(keepPrev === false && { data: undefined }),
      });
      ...
      if (err) {
        ...
        defaults?.onSuccess?.(finalData);
        configsRef.current[index].onSettled?.(finalData, null);
        defaults?.onSettled?.(finalData, null);
      }
