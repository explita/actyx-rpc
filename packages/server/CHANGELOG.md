...
- **Package Architecture Split**: Extracted React hooks, query caching, SSE/WebSocket client connectors, and provider infrastructure into the standalone `@explita/actyx-rpc-react` package.
- **Removed `./react` and `./client` subpaths**: `@explita/actyx-rpc` is now a dedicated, backend-only server RPC runtime. React consumers should install and import from `@explita/actyx-rpc-react`.

### Added

- **Native Next.js 15+ Async Support in `webRoute`**:
  - `webRoute` automatically resolves asynchronous route parameters (`options.params: Promise<any>`) introduced in Next.js 15/16.
  - `webRoute` automatically resolves asynchronous search parameters (`options.searchParams` and `options.queryParams: Promise<any>`), seamlessly mapping them to `input.query`.
  - `webRoute` automatically resolves asynchronous headers and cookies (`options.headers`, `options.cookies: Promise<any> | (() => Promise<any>)`), preserving resolved values in procedure context.
  - Resilient `req.headers` resolution compatible with standard Web `Headers`, Promises, and async getters.
- **Isolated `input.params` and `input.query` in `webRoute`**:
  - Instead of flattening and clobbering query string parameters into the body, `input` provides isolated properties:
    - `...bodyParams`: JSON / FormData payload fields at the root of `input` (for POST/PUT/PATCH).
